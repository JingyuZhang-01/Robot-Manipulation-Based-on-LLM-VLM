...
def set_lmp_objects(lmps, objects): #让每个语言模型程序（LMP）知道自己可以操作哪些对象
...
def load_prompt(prompt_fname):
    ...
    # 原始路径
    if '/' in prompt_fname:
        prompt_parts = prompt_fname.split('/')
        original_path = os.path.join(curr_dir, 'prompts', *prompt_parts)
        possible_paths.append(original_path)
    
    # 直接放在 prompts 目录下的路径
    direct_path = os.path.join(curr_dir, 'prompts', prompt_fname)
    possible_paths.append(direct_path)
    
    # 如果包含子目录但文件可能直接放在 prompts 下，只取文件名
    if '/' in prompt_fname:
        filename = prompt_fname.split('/')[-1]
        filename_only_path = os.path.join(curr_dir, 'prompts', filename)
        possible_paths.append(filename_only_path)
    ...
    # 如果所有路径都失败，抛出详细的错误信息
    error_msg = f"Could not find prompt file: {prompt_fname}\n"
    error_msg += f"Tried the following paths:\n"
    for path in possible_paths:
        error_msg += f"  - {path}\n"
    error_msg += f"Current directory: {curr_dir}\n"
    error_msg += f"Prompts directory: {os.path.join(curr_dir, 'prompts')}"
